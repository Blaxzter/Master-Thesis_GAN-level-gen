# The content of this file are taken from the
# Level Generation for Angry Birds with Sequential VAE and Latent Variable Evolution project.
# https://github.com/yoshinobc/Level-Generation-for-Angry-Birds-with-Sequential-VAE-and-Latent-Variable-Evolution/blob/main/converter/xml2txt.py
...
class xml2txt(object):
        ...
        for text in xml_txt:
            ...
            if len(content) >= 2:
                if content[0] == "Block":
                    ...
                    block_name = content[0]
                    x_idx = [i for i, e in enumerate(content) if 'x=' in e][0]
                    x, y = float(content[x_idx][3:-1]), float(content[x_idx + 1][3:-1])
                else:
                    continue
                x = round(x, 1)
                x_min = min(x, x_min)
                contents.append([block_name, x, y, material])
        return contents, x_min
    ...
    def xml2vector_core(self, file_name, is_pre):
        f = open(file_name, "r")
        xml_list = f.read()
        contents, x_min = self.load_xml(xml_list)
        if len(contents) <= 5:
            return None
