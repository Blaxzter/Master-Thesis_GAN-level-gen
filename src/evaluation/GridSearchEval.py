...
class GridSearchEval:
    def __init__(self):
        ...
        self.selected_parameters = ['round_to_next_int', 'custom_kernel_scale', 'minus_one_border', 'combine_layers']
        self.negative_air_value = 0
        self.cutoff_point = 0.8
        ...
        self.parameter_dict = dict(
            round_to_next_int = dict(type = 'bool', default = False, values = [True, False],
                                     label = 'Round To Next Int'),
            custom_kernel_scale = dict(type = 'bool', default = True, values = [True, False],
                                       label = 'Custom Kernel Scale'),
            minus_one_border = dict(type = 'bool', default = False, values = [True, False], label = 'Minus One Border'),
            combine_layers = dict(type = 'bool', default = False, values = [True, False], label = 'Combine Layers'),
            negative_air_value = dict(type = 'number', default = -2, values = [-10, -5, -2, -1, 0],
                                      label = 'Negative Air Value'),
        ...
        self._app.layout = html.Div([
            dcc.Markdown("### Grid Search Visualisation"),
            html.Div([
                html.Div([
                    dcc.Markdown("#### Single Parameter"),
                    dcc.Checklist(
                        id = "bool_parameters",
                        options = [
                            {"label": 'Round To Next Int', "value": 'round_to_next_int'},
                            {"label": 'Custom Kernel Scale', "value": 'custom_kernel_scale'},
                            {"label": 'Minus One Border', "value": 'minus_one_border'},
                            {"label": 'Combine Layers', "value": 'combine_layers'},
                        ],
                        value = self.selected_parameters
                    ),
                    html.Div([
                        html.Div([
                            dcc.Markdown('Negative Air Value'),
                            dcc.Slider(-10, 0, id = 'negative_air_value', step = None,
                                       marks = {-10: '-10', -5: '-5', -2: '-2', -1: '-1', 0: '0'},
                                       value = self.negative_air_value),
                        ], style = {'flex-grow': '1'}),
                        html.Div([
                            dcc.Markdown('Cutoff Point'),
                            dcc.Slider(0.1, 0.95, id = 'cutoff_point', step = None,
                                       marks = {0.1: '0.1', 0.5: '0.5', 0.8: '0.8', 0.95: '0.95'},
                                       value = self.cutoff_point)]
                            , style = {'flex-grow': '1'})
                    ], style = {'display': 'flex'}),
                    dcc.Markdown("#### Set with optimized value"),
                    dcc.Checklist(
                        id = "optimized_value",
                        options = [
                ...
                html.Div([
                    ...
                    dcc.Markdown("#### Biggest Difference"),
                    dcc.Dropdown(
                ...
                html.Div([
                    dcc.Markdown("#### Compare Value"),
                    dcc.Checklist(['Compare Values'], [], id = 'compare_values'),
                    dcc.RadioItems(
                        ...
                        options = [
                            ...
                            {"label": 'Minus One Border', "value": 'minus_one_border'},
                            {"label": 'Combine Layers', "value": 'combine_layers'},
                            {"label": 'Negative Air Value', "value": 'negative_air_value'},
                            {"label": 'Cutoff Point', "value": 'cutoff_point'},
                        ],
                        value = 'round_to_next_int',
                        inline = True
                    ),
